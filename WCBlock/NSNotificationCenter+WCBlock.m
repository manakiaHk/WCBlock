...
- (void)dealloc {
    ...
    [WCNotificationCenter removeObserver:self.notifObserver name:self.notifName object:self.notifObj];
}
...
-(void)wc_addObserverForName:(NSString*)name
    ...
    target.notifObserver =  [WCNotificationCenter addObserverForName:name object:obj queue:queue usingBlock:^(NSNotification * _Nonnull note) {
...
-(void)wc_addObserverForName:(nonnull NSString*)name
    ...
    target.notifName = name;
    target.notifBlock = block;
    [WCNotificationCenter addObserver:target selector:@selector(notifReceived:) name:name object:obj];
}
